...
如果你实际回答的评估题形成了值得以后回看的案例、判断或错误校正，Agent 会先把它简化成 Markdown 引用块，并询问是否纳入最终笔记。例如：

```markdown
> **节点评估摘录：量纲如何影响 KNN 距离**
>
> **情境：** 年龄和年收入未经缩放直接计算欧氏距离。
>
> **回答要点：** 收入的数值跨度更大，会主导距离。
>
> **结论与校正：** 距离模型会受特征尺度影响，应使用训练集确定缩放参数并对训练、测试数据一致变换。
```

只有你明确同意，这段摘录才会进入当前 Quest 的候选笔记材料；说“不记录”或“下一步”会直接丢弃，不形成缺口或待办。节点阶段不会提前修改正式笔记，仍要在实际应用和综合评估完成后，由 `distill` 展示包含摘录的完整草稿并取得最终写入确认。直接放行没有实际问答，因此不会生成摘录。
...
`## 关系` 固定放在一级标题后的第一个二级章节，让读者打开笔记就能看到它的前置、来源、应用、对比、所属 Topic Map 和相关 Learning Quest，再进入核心命题。YAML 仍位于一级标题之前。

不会机械生成“我的理解”“理解验证”“来源与证据”等学习过程章节。掌握证据保存在 `.learning/` 或 `90-学习会话/`；只有你在节点评估后明确批准的精炼摘录，才会作为引用案例嵌入知识正文，而且不会携带评分或通过状态。
