...
2. 把内容穷尽拆成带原文位置的原子知识点，包括概念、条件、限制、方法、数值、例子、反例、图片说明、疑问和待验证判断，并建立覆盖账本；
3. 检索已有正式知识，避免同义重复；
4. 展示一次完整整合计划；
5. 在你批准后执行计划内的创建和更新；
...
重复内容可以标记“已合并”，但每个原始位置仍要有去向；疑似错误、疑问、反例和不完整内容必须保留为待核查，不能自动丢弃。“明确丢弃”只适用于纯排版噪声、无知识含义的残留或你明确要求不要保留的内容。存在任何“未处理”项时，原始输入必须留在原位。
...
只处理用户明确指定的 `00-原始笔记/` 下的 Markdown 文件。输入文件是本次转换的唯一内容依据，但不是证明其中事实正确的外部证据；系统也不会在没有完整覆盖的情况下清理输入。

转换采用封闭语料规则：必须穷尽记录原始笔记里的定义、条件、限制、步骤、参数、数值、例子、反例、疑问和图片说明，并用覆盖账本把每一点映射到正式笔记中的具体位置或明确的保留状态；不能用概括性总结吞掉细节。与此同时，Agent 只能重组、去重和改写原始内容，不能补写笔记之外的新定义、机制、案例、代码、参数或结论，也不能把已有 Vault 笔记或模型常识混入转换结果。材料不足以满足正式笔记规范时保持待补充，而不是自行扩写。完整边界见 [原始笔记的封闭语料转换边界](docs/raw-note-conversion-boundary.md)。

原始笔记中的每张图片必须在下一物理行提供一段完整包在英文括号 `(...)` 或中文括号 `（...）` 中的图片说明，图片和说明之间不能有空行。Agent 不依赖直接识图，而是依据这段用户说明判断图片中的对象、数值、关系、现象和用途：
...
装饰图片也写成 `（装饰性图片，不承载知识内容。）`。如果某张图片下一行没有说明、括号不匹配、内容为空或只有“见图”等无效描述，Agent 会一次性列出笔记路径、图片行号和图片名，提醒补充；不会猜图，也不会把该批次判为 100% 覆盖或清理原始输入。完整规则见 [原始笔记图片说明约定](docs/raw-note-images.md)。

图片和说明在整合过程中始终作为一个不可拆分单元。只要它承载的知识被写入 Concept Note、方法或实践等正式笔记，正式笔记也必须保留“图片行 → 紧邻下一行括号说明”的形式，并把它放在实际解释的机制或案例附近。不能只摘录说明、只保留图片或集中堆到文末；附件路径无法稳定解析时，该单元继续保持未处理。

写入前展示计划；写入后回读验证；达到 100% 覆盖后，只能把输入可恢复地移动到 `.trash/`。

完成忠实转换后，Agent 可以在聊天中另列“建议 / 延伸 / 质疑与待核查”，但必须标明这些内容没有写入正式笔记、不属于本批次输入、也不计入覆盖率。你选择其中一项后，再作为新的学习、核查或更新任务处理。
...
不自动成为。`integrate-learning` 会先穷尽拆分、建立覆盖账本、查重和决定去向。宽主题通常进入 Topic Map，操作步骤通常进入实践与产出，边界清楚且原始材料足够的命题才适合成为 Concept Note；材料不足时不会靠 Agent 扩写补齐，而是完整保留并在转换后提出补充建议。

### 原始笔记里有图片怎么办？

每张图片下一行写一段由 `(...)` 或 `（...）` 完整包围的内容说明。Agent 根据说明进行整合，不要求模型能够直接识图；缺少或无法支撑理解的说明会被集中列出并要求补充，在修复前相关内容保持未处理，原始笔记不会被清理。图片内容被晋升到正式笔记时，图片和说明会一起写入并保持物理相邻，不会只留下其中一项。
