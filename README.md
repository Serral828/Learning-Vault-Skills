...
`teach` 是一个 Quest 内的教学总控入口。开始时，`learning-hub` 会为新目标创建或复用稳定 Quest ID；教学过程中每个节点都经过一次 `assess`，全部节点通过后完成实际应用输出并再做一次综合 `assess`，通过后才进入 `distill`；最后由 `learning-hub` 保存状态并给出可在未来执行的动作。
...
每个规划出的教学节点都有完成标准。讲解和示例结束后，该节点只是“待评估”；`teach` 必须在进入下一节点前转入 `assess`，用实际电机控制案例检查你能否诊断 PID 参数问题。评估不会只让你复述定义，而会加入噪声、延迟、饱和、异常结果或指标冲突，让你完成预测、诊断、设计或权衡。

这不意味着每讲一句话就提问。微小知识点会合并成一个可评估节点，每个节点通常只给一个核心任务。如果你已经在自然对话中主动完成了等价的新情境应用，`assess` 会直接评价这段回答并记录证据，不重复出题；没有提问也没有自然证据时，节点不能被算作完成。
...
所有必需节点分别通过后，`teach` 不会立即生成笔记。你需要完成一个联合使用这些节点的实际成果，例如可运行控制程序、硬件诊断方案、完整演算、实验设计或带约束的决策分析。已有真实项目能够覆盖完成标准时，可以直接作为输出，不需要重复造练习。

输出完成后，`teach` 再调用一次 `assess` 做综合评估。这次不会简单汇总节点分数，而会检查成果是否真正连接多个节点、关键决策能否解释、条件改变后能否诊断，以及整个 Quest 的完成标准是否达到。综合评估未通过时返回具体节点和输出缺口，不进入笔记生成。

#### 第四步：把已掌握内容沉淀为正式知识

综合评估通过后，`teach` 才会在正常收束前主动转入 `distill`，提议把在最终成果中再次得到支持的概念整理成候选笔记。你不需要在课程结束后提醒它生成笔记，也不需要另发一次 `/skill:distill`。
...
- 读取已有学习证据，不把用户默认成零基础；
- 生成当前目标所需的最小知识路径；
- 一次教授一个可评估节点，讲解结束后先取得证据再进入下一节点；
- 需要事实核查时转入 `verify`；
- 每个规划节点边界都转入 `assess`；已有等价证据时直接判级，没有证据时提出一个新情境任务；
- 全部节点通过后要求用户完成实际应用输出，再调用一次 `assess` 做综合评估；
- 只有综合评估通过，才在正常收束前主动调用 `distill` 并展示候选草稿。
...
节点评估返回节点名称、目标等级、实际证据等级和节点状态，但不会产生 Distill 资格。全部节点通过并完成实际应用输出后，综合评估才返回整个 Quest 是否通过、各节点在成果中的证据和 `distillReady`。教师讲过的案例不是用户证据；用户已有的真实作品可以直接接受综合评估。

### `distill`：晋升正式概念笔记

正式 Concept Note 必须同时满足：

1. 当前 Quest 的全部必需节点已经分别通过评估；
2. 用户已经完成覆盖这些节点的实际应用输出；
3. 综合评估已经通过；
4. 用户已经用自己的语言解释核心机制；
5. 关键事实有来源，或明确标记为个人推断；
6. 笔记边界足够小，且只有一个核心命题。

不满足时保留在学习会话中，不把 AI 生成的流畅文本伪装成已掌握知识。

用户审阅候选稿或点击“确认写入”不属于掌握证据。缺少节点评估、实际应用输出或综合评估中的任一步时，系统只能返回最早缺失的阶段，不能提供“先审阅候选 Concept Note”作为绕过路径。

每篇新晋升的 Concept Note 还会进行一次 Topic Map 路由。系统会选择最近的、可长期容纳多个概念的上位知识面，例如 `KNN` 进入“机器学习”，而不是创建“KNN 地图”。已有合适地图时提议更新；没有时必须同时展示一个最小 Topic Map 草稿并提议创建，不能只检查后不了了之。Concept Note 与地图会用双向 Wikilink 连接；你可以分别确认笔记和地图，暂时不处理地图时该动作会保存在 Learning Hub。
...
Topic Map 不要求事先存在。新概念晋升时，Agent 会先复用最合适的已有地图；若不存在，就提出创建一个宽泛、最小可用的地图，并至少收录本次新笔记。地图只有一个入口也可以，后续知识会逐步扩展它；系统不会生成虚假的占位链接。完整规则见 [Topic Map 创建与归属规范](docs/topic-map-writing.md)。
...
实际应用输出 → 联合使用全部必要节点
assess → 综合评估整个 Quest
distill → 综合评估通过后提议生成正式笔记
link-knowledge → 新笔记晋升后提议建立关系
learning-hub → 保存 Quest 状态和未来动作
```

其中评估和 Distill 不是等你事后提醒的可选收尾：每个规划节点在进入下一节点前进行一次节点评估；所有节点通过后完成实际应用输出，再进行一次综合评估；综合评估通过后，`teach` 才在正常收束前主动展示 Distill 草稿并询问是否写入。只有你明确表示现在必须停止时，系统才把“待节点评估”“待实际应用输出”“待综合评估”或“待 Distill”保存到 Learning Hub，留到下次按顺序完成。
